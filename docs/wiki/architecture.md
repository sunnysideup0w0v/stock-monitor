...
> 최종 업데이트: Phase 5 완료 기준 (2026-05-17)
...
│   └── SettingsView          — 설정 창 (SwiftUI, 7개 탭)
│
├── 코어 계층
...
│   ├── KRXManager            — KRX/네이버 전종목 시세 수집 (stock_universe)
│   ├── ScreenerEngine        — 스크리너 GRDB 쿼리 빌더
│   └── ClaudeAnalyzer        — Anthropic Claude AI 분석 (SSE 스트리밍, actor)
│
├── 어댑터 계층
│   ├── KISAdapter            — 한국투자증권 REST/WebSocket (actor)
│   ├── KiwoomAdapter         — 키움증권 REST (actor)
│   ├── MiraeAssetAdapter     — 미래에셋증권 (Stub, 미구현)
│   └── MockBrokerAdapter     — 오프라인 개발용 더미
│
├── 데이터 계층
│   └── DatabaseManager       — GRDB SQLite, Migration v1~v10
│
└── 인프라
    ...
    ├── AppLogger             — os.Logger + 파일 이중 로깅
    └── CrashLogger           — ObjC 예외 + Swift 시그널 크래시 로그
```
...
| `.krxDataUpdated` | KRXManager (업데이트 완료 후) | ScreenerView 등 | 스크리너 종목 유니버스 갱신 알림 |
